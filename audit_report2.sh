...
# ==========================================
export MASTER="origin/master"
export DEVELOP="origin/develop"
export QA="origin/qa"
OUTPUT="audit_report.html"
FETCH=true
export STALE_THRESHOLD=30 # Days
...
while [[ "$#" -gt 0 ]]; do
  case $1 in
    ...
    -h|--help) 
      echo -e "${C_BLUE}Git Branch Audit Tool (Parallel Edition)${C_NC}"
      exit 0 
      ;;
...
if ! git rev-parse --is-inside-work-tree &> /dev/null; then echo "❌ Run inside a git repository."; exit 1; fi
...
process_branch() {
  ...
  # Output as a single line for easy parsing
  if [ "$ON_MASTER" -eq "0" ]; then
    echo "VIOLATION|<tr class='violation'><td><span class='badge red'>${BRANCH##origin/}</span></td><td>from ${SOURCE}</td><td>${CREATOR}</td><td>${DATE}</td><td>${STALE_BADGE}</td><td>${COMMITS}</td><td><code>${FORK_SHA}</code></td></tr>"
  else
    echo "OK|<tr class='ok'><td><span class='badge green'>${BRANCH##origin/}</span></td><td>from master</td><td>${CREATOR}</td><td>${DATE}</td><td>${STALE_BADGE}</td><td>${COMMITS}</td><td><code>${FORK_SHA}</code></td></tr>"
  fi
}
...
# ⚡ EXECUTE IN PARALLEL
# ==========================================
echo -e "${C_CYAN}⚡ Analyzing 1000+ branches simultaneously (8 threads)...${C_NC}"
FEATURE_BRANCHES=$(git branch -r | grep -v "HEAD\|master\|develop\|qa\|release\|hotfix" | sed 's/^ *//')

# Run using xargs with -P 8 (8 concurrent processes)
RESULTS=$(echo "$FEATURE_BRANCHES" | xargs -n 1 -P 8 -I {} bash -c 'process_branch "{}"')

# Parse results
VIOLATIONS=$(echo "$RESULTS" | grep "^VIOLATION|" | sed 's/^VIOLATION|//')
OK_BRANCHES=$(echo "$RESULTS" | grep "^OK|" | sed 's/^OK|//')

VIOLATION_COUNT=$( [ -z "$VIOLATIONS" ] && echo 0 || echo "$VIOLATIONS" | wc -l | tr -d ' ' )
OK_COUNT=$( [ -z "$OK_BRANCHES" ] && echo 0 || echo "$OK_BRANCHES" | wc -l | tr -d ' ' )
STALE_COUNT=$(echo "$RESULTS" | grep -c "Stale (" || echo 0)
...
<head>
  ...
  <style>
    ...
    body { font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;
           max-width: 1200px; margin: 40px auto; padding: 0 24px; background: var(--bg); color: var(--text); font-size: 14px; }
